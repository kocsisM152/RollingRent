...
const CarDetail = () => {
  ...
  const formattedPrice = (car.ar && typeof car.ar === 'number')
    ? car.ar.toLocaleString('hu-HU', { style: 'currency', currency: 'HUF', maximumFractionDigits: 0 })
    : `${car.ar || 'N/A'} Ft`;
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden transform transition duration-500">
        ...
        <div className="p-6 sm:p-10 border-b border-gray-100">
          <h1 className="text-4xl font-extrabold text-gray-900 mb-2 leading-tight">
            {car.marka} <span className="text-indigo-600">{car.tipus}</span>
          </h1>
          <p className="text-xl text-gray-500">
            Évjárat: {car.evjarat || 'N/A'} | Ár: <span className="font-bold text-green-600">{formattedPrice}</span>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-indigo-50 p-6 rounded-xl shadow-lg">
              ...
              <ul className="space-y-3 text-gray-700">
                ...
                <DetailItem label="Teljesítmény" value={`${car.teljesitmeny || 'N/A'} LE`} icon="🚀" />
                <DetailItem label="Urtartalom" value={`${car.urtartalom || 'N/A'} cm³`} icon="🏎️" />
                <DetailItem label="Származási ország" value={car.szarmazasiorszag || 'N/A'} icon="🌍" />
                <DetailItem label="Foglalható" value={car.foglalhatoe ? 'Igen' : 'Nem'} icon={car.foglalhatoe ? '✅' : '❌'} />
              </ul>
            </div>

            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md">
              <h2 className="text-2xl font-bold text-gray-800 mb-3">Leírás</h2>
        ...
        <div className="p-6 sm:p-10 bg-gray-100 flex justify-center">
          ...
          >
            {car.foglalhatoe ? 'Autó Foglalása' : 'Jelenleg nem foglalható'}
          </button>
        </div>
      </div>
    </div>
